...
export const EXCEL_VALIDATION_ERROR = "Tarkista että ELSA:sta hakemasi excel-tiedoston sarakkeita ei ole muokattu ja että tarvittavat sarakkeet on tiedostossa (Sarja, Pvm, Klo, Kenttä, Koti, Vieras)."
export const EXCEL_DATE_FORMAT_ERROR = (date: string | number) => `Odottamaton päivämäärämuoto: ${date}`
...
export const formatErrorMessage = (error: unknown): string => {
  return `Virhe tiedoston prosessoinnissa: ${error instanceof Error ? error.message : String(error)}`
}
